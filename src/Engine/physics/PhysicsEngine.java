...
//                System.out.println("block!");
    ...
    // todo: corner normals calculating
    private Vector3f calcNormal(Vector3f colVec){
        ...
        if(xEntry == yEntry){
            return new Vector3f((colVec.getX() > 0) ? 1 : -1, (colVec.getY() > 0) ? 1 : -1, 0);
        }

        if(xEntry == zEntry){
            return new Vector3f((colVec.getX() > 0) ? 1 : -1, 0, (colVec.getZ() > 0) ? 1 : -1);
        }

        if(zEntry == yEntry){
            return new Vector3f(0, (colVec.getY() > 0) ? 1 : -1, (colVec.getZ() > 0) ? 1 : -1);
        }
